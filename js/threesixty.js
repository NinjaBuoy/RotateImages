...
$(document).ready(function () {
	...
	*/
	function loadImage() {
		...
		var imageName = "img/DAC_Phase2_Aerial_" + zeroFill(loadedImages, 4) + ".png";
		/*
	...
	*/
	function trackPointer(event) {
		...
		if(userDragging || demoDragging || sliderDrag) {
			...
			//Not the best way
			// if(sliderDrag){
			// 	endFrame = Math.trunc(slider.value * 10);
			...
			// if(!sliderDrag){
			// 	rem = endFrame%10
			// 	if(rem>4) endFrame = Mathf.trunc(endFrame - rem)+10;
			// 	else endFrame = endFrame -rem;
	...
	//$(function() {
		...
		// endFrame = Math.trunc((totalFrames * slider.value)/ slider.max);
		// console.log(endFrame);
	// });
});